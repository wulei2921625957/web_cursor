...
export function App({
      ...
      : [
          {
            ...
            kind: "status",
            label: "apikey",
            text: "No CURSOR_API_KEY found. Type /set_apiKey or /set_apiKey --save, then paste your key.",
          },
        ]
  )
  ...
  useKeyboard((key) => {
    ...
    if (pendingSecretInput && key.name === "escape") {
      setPendingSecretInput(null)
      setInput("")
      setMode("input")
      return
    }
  ...
  const commandItems = useMemo<CommandSelectItem[]>(
    () =>
      getSlashCommandItems(input).map((item) => {
        ...
        return {
          key: item.key,
          name: description ? `${name ?? item.value}  ${description}` : name ?? item.value,
          description: "",
          value: item.value,
        }
      }),
  ...
  const submitInput = (value: string) => {
    ...
    if (pendingSecretInput?.kind === "apiKey") {
      if (!prompt) {
        addEntry(
          "status",
          "apikey",
          "Paste CURSOR_API_KEY and press Enter, or press Escape to cancel."
        )
        return
      }

      const save = pendingSecretInput.save
      setPendingSecretInput(null)
      void setApiKeyValue(prompt, save)
      return
    }
  ...
  const selectCommand = (item: CommandSelectItem) => {
    ...
    const command = getSlashCommand(rawCommand)
    void runCommand(command === item.value ? rawCommand : item.value)
  }
  ...
  const setApiKeyFromCommand = async (rawCommand: string) => {
    if (busy) {
      return
    }

    const parsed = parseSetApiKeyCommand(rawCommand)

    if (!parsed.apiKey) {
      setPendingSecretInput({ kind: "apiKey", save: parsed.save })
      addEntry(
        "status",
        "apikey",
        "Paste CURSOR_API_KEY and press Enter. It will not be added to the transcript."
      )
      return
    }

    await setApiKeyValue(parsed.apiKey, parsed.save)
  }

  const setApiKeyValue = async (apiKeyValue: string, save: boolean) => {
    const session = sessionRef.current
    if (!session || busy) {
      return
    }
    ...
    try {
      await session.setApiKey(apiKeyValue)

      if (save) {
        persistApiKey(apiKeyValue)
      }
      ...
      addEntry(
        ...
        "apikey",
        save
          ? "API key updated for this session and saved to user environment."
          : "API key updated for this session."
      )
    } catch (error) {
      addEntry("error", "apikey", getErrorMessage(error))
    } finally {
      setBusy(false)
    }
  }

  const persistApiKey = (apiKey: string) => {
    ...
    <box flexDirection="column" height={rows} paddingX={1}>
      ...
      {mode === "command" ? (
        <box
          ...
          paddingX={1}
        >
          <text attributes={TextAttributes.BOLD}>命令</text>
          <text fg="gray">方向键选择，Enter 执行，Escape 取消。</text>
          <select
          ...
          <TuiInput
            ...
            placeholder={
              busy
                ...
                : pendingSecretInput
                  ? "Paste CURSOR_API_KEY and press Enter"
                : "Ask or type /help"
            }
            value={input}
            onInput={(value) => {
              setInput(value)
              if (!busy && !pendingSecretInput && value.startsWith("/")) {
                setMode("command")
              }
            }}
...
function applyAgentEvent(
  ...
  switch (event.type) {
    ...
    case "compaction":
      return upsertEntry(
        items,
        `compaction-${assistantId}`,
        event.status === "failed" ? "error" : "status",
        "context",
        formatCompactionEvent(event)
      )
    case "thinking": {
...
function formatCompactionEvent(event: Extract<AgentEvent, { type: "compaction" }>) {
  const details = [
    event.status,
    event.message,
    event.originalChars ? `original=${event.originalChars}` : undefined,
    event.summaryChars ? `summary=${event.summaryChars}` : undefined,
    event.retainedChars ? `retained=${event.retainedChars}` : undefined,
  ].filter(Boolean)
...
function parseSetApiKeyCommand(rawCommand: string) {
